...
def restaurantCategoryDist(restaurants):
        #Breaking up categories so that each row has one category. 1 -1 relation
        #This is as known as splitting (Exploding) a column into multiple rows
        ...
        final = new_restaurants.groupby(['category'], as_index=False).count().sort_values('#restaurants',ascending=False)
        ...
        final_format = final.applymap(str)
